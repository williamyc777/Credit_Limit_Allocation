...
\begin{itemize}
    ...
    \item \textbf{Classification at 0.5 threshold:} Accuracy is dominated by the majority class; \textbf{recall on defaults} is \textbf{low} under the default threshold---expected under imbalance when no threshold tuning or class weights are used.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Supporting scripts:} \texttt{src/eda\_pd\_distribution.py}, \texttt{src/causal\_pd\_analysis.py} (illustrative PD sensitivity to \texttt{loan\_amnt}); \texttt{run\_pipeline.sh} and \texttt{package\_output.sh}.
...
\begin{itemize}
    ...
    \item \textbf{Model alignment:} PD vs.\ \texttt{loan\_amnt} and ROC analysis indicate the baseline produces a sensible \textbf{ranking} signal for risk, even if binary decisions at 0.5 are not tuned for default capture.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Class imbalance:} High accuracy masks poor default recall at 0.5; for \textbf{business metrics} we rely on PD as a continuous input and will validate policy choices in simulation rather than only accuracy at a fixed threshold.
