...
\begin{itemize}
    \item \textbf{Algorithms compared:} Logistic regression, decision tree, and random forest (\texttt{scikit-learn}); logistic uses \textbf{standardized} features; best model by hold-out \textbf{ROC-AUC} is saved for scoring and simulation.
...
\begin{itemize}
    ...
    \item \textbf{PD models:} \texttt{src/train\_model.py} --- multiple models, AUC and PR-AUC, \texttt{best\_model.pkl} and \texttt{pd\_predictions.csv}.
    \item \textbf{Supporting scripts:} \texttt{src/eda\_pd\_distribution.py}; \texttt{src/causal\_pd\_analysis.py}; \texttt{src/decision\_logic.py}; \texttt{src/simulation\_profit.py}; \texttt{run\_pipeline.sh} (auto-selects Anaconda Python when available); \texttt{package\_output.sh}; \texttt{PROJECT\_STATUS.md} (checklist).
...
\begin{itemize}
    ...
    \item The \textbf{prescriptive path} (re-score under counterfactual \texttt{loan\_amnt}, expected profit on a grid) is implemented in code; we may add \texttt{class\_weight}, richer profit accounting, or stronger learners (e.g., gradient boosting) as refinements for the final report.
\end{itemize}
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Align \textbf{profit parameters} and narrative with the business case; optionally add PR-curve figures alongside ROC for the minority default class.
